...
class ExternalLocation(BaseSecurable):
    ...
    url: str = Field(..., min_length=10, max_length=1024, description="Storage URL (s3://, abfss://, gs://)")
    storage_credential: StorageCredential = Field(..., description="Associated storage credential for authentication")
    ...
    def to_sdk_update_params(self) -> Dict[str, Any]:
        ...
        return {"name": self.resolved_name, "url": self.url, "comment": self.comment}
