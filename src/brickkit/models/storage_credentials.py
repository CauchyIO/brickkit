...
class AwsIamRole(BaseModel):
    ...
    role_arn: str = Field(..., pattern=r"^arn:aws:iam::\d{12}:role/[\w+=,.@/-]+$", description="AWS IAM role ARN")
...
class GcpServiceAccountKey(BaseModel):
    ...
    email: str = Field(
        ..., pattern=r"^[a-z0-9-]+@[a-z0-9-]+\.iam\.gserviceaccount\.com$", description="Service account email"
    )
...
class StorageCredential(BaseSecurable):
    ...
    is_existing: bool = Field(
        default=False, description="If True, credential already exists and name won't get environment suffix"
    )
    ...
    def to_sdk_update_params(self) -> Dict[str, Any]:
        ...
        if self.aws_iam_role:
            params["aws_iam_role"] = {"role_arn": self.aws_iam_role.role_arn}
